  ...
  return (
    <>
      <div className="flex flex-col justify-around p-3 gap-2 bg-gradient-to-r from-green-400 to-lime-400 rounded-xl shadow-lg text-white w-11/12 sm:w-5/12 lg:w-3/12">
        <div className="">
          <img src={item.image} alt={item.name} className=" rounded-xl object-cover w-full h-48 bg-no-repeat bg-center" loading="lazy"/>
        </div>
        ...
        <div className="flex flex-row justify-between text-lime-900">
          <div>{item.price} $ </div>
          <div className="flex flex-row gap-1 items-center">{item.review} <FaStar /> </div>
        </div>
